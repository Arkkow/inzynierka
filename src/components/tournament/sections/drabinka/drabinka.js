...
export const Drabinka = (props) => {
  ...
  return (
    <>
      ...
      >
        {/*  jeśli zalogowany user to organizator && jeśli turniej nie jest zakończony*/}
        {props.user.role == props.calendar_list.creator &&
        props.calendar_list.approved !== 3 ? (
          props.calendar_list.approved === 2 ? (
            props.calendar_list.typeOfLadder == "DRABINKA O MIEJSCA" ? (
              <div>
                <End_tournament_places_popup {...props} />
              </div>
            ) : (
              <div>
                <EndTournament_popup {...props} />
              </div>
            )
          ) : (
            <div>
              <EndUnrankedTournament_popup />
            </div>
          )
        ) : null}
      </div>
